...
class UNet3Plus(nn.Module):
    ...
    def _decoder_layer(self, in_channels, pool_size):
        layers = []
        if pool_size > 1:
            layers.append(nn.MaxPool2d(pool_size))
        layers.append(nn.Conv2d(in_channels, 32, kernel_size=3, padding=1))  # 调整通道数为64
        layers.append(nn.ReLU(inplace=True))
        return nn.Sequential(*layers)

    def _upsample_layer(self, in_channels, scale_factor):#上采样
